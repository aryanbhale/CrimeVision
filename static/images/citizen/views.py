...
def loginView(request):
	if request.method == "POST":
		...
		if user is not None:
			login(request, user)
			if user.is_admin:
				messages.success(request, 'Hi ' + request.user.name + ',  Welcome to the Crime Vision Portal. Access and manage authorized public safety services securely through our online platform.')
				return redirect('police_home')
			else:
				messages.success(request, 'Hi ' + request.user.name + ',  Welcome to Crime Vision. This portal enables citizens to access, submit, and track designated services through a secure digital platform.')
				return redirect('citizen_home')
		else:
			messages.error(request, ' Invalid ! Re-entering wrong credentials constitutes to permenant prevention of access.')
			return redirect('login')
	context = {}
