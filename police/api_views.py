...
def add_record(request):
	...
	# Build a mutable dict — request.data is an immutable QueryDict
	key_points = generate(img_str)
	if not key_points:
		return Response({"detail": "No face detected in the uploaded image."})
	data = {
	...
	serializer = CrimeRecordsSerializer(data=data)
	if serializer.is_valid():
		serializer.save()
		return Response({"detail": "record added successfully."})
	return Response({"detail": "something went wrong.", "errors": serializer.errors})
...
def match_image(request):
	...
	# result is [] (empty list) when no match — 'not result' is True for [], so check explicitly
	if result is None or result == "None" or result == []:
	...
	for res in result:
		...
		except CrimeRecords.DoesNotExist:
			response_data[pk] = {'name': 'Unknown', 'against': '0', 'img': ''}
